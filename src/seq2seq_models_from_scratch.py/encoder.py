...
class Encoder:
    def __init__(self,input_dim,hidden_dim,loss):
            self.encoder= self.Encoder_layer(input_dim,hidden_dim)
    ...
    class Encoder_layer:
        ...
        def  encoder_optimizer(self,learning_rate,optimizer):
    ...
    def train(self, X, labels, learning_rate=0.01, epochs=50, optimizer='sgd'):
        for epoch in range(epochs):
            for sentence,sentence_label in zip(X,labels):
                # Forward through RNN
                final_hidden = self.encoder.forward(sentence)
                ...
                input_grad = self.net.backward(sentence_label, prediction)
